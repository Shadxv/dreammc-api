...
public class NPCMoveHereCommand extends PaperSubcommand {

    private final NPCTestCommand npcTestCommand;

    public NPCMoveHereCommand(NPCTestCommand npcTestCommand) {
        super("movehere",
                "",
                "",
                List.of(),
    ...
    public @NotNull ICommandResponse execute0(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String[] args) {
        ...
        this.npcTestCommand.getNpcs().get(player).getGoalSelector().getGoalComponent(MovementComponent.class).setTargetLocation(player.getLocation());
